...
static void init(void) {
	...
	state.world = tics_world_create((tics_world_desc){
		...
		.air_friction_linear = 0.02f,
		.air_friction_angular = 0.95f
	});
	...
	// Generate tightly packed hexagonal ground grid
	float z_spacing = sqrtf(3.0f) * SPHERE_RADIUS;
	float x_spacing = 2.0f * SPHERE_RADIUS;
	...
	for (int z = -half_ext; z <= half_ext; z++) {
		for (int x = -half_ext; x <= half_ext; x++) {
			...
			float offset_x = (abs(z) % 2 == 1) ? SPHERE_RADIUS : 0.0f;
			su_vec3 pos = {(x * x_spacing) + offset_x, 0.0f, z * z_spacing};
