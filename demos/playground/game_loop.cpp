...
GameLoop::GameLoop(std::function<void(float, float)> variable_update,
	...
	: m_variable_update(variable_update), m_fixed_update(fixed_update),
	  fixed_delta(fixed_update_delta), variable_delta_min(variable_update_delta_min),
	  variable_delta_max(variable_update_delta_max),
	  m_last_update_start(std::chrono::high_resolution_clock::now()), m_fixed_delay(0.0f) {}
...
void GameLoop::update() const {
	...
	if (delta > variable_delta_max) { delta = variable_delta_max; }
	...
	if (delta < variable_delta_min) {
		...
		auto destination_time =
			start_time + std::chrono::duration_cast<std::chrono::nanoseconds>(wait_time);
