...
static tics_vec3 get_velocity(rigid_body_data* rb, tics_vec3 point) {
	...
	bool no_rotation = tics_vec3_length(q_vec) < 0.01f;

	tics_vec3 axis =
		no_rotation ? (tics_vec3){1, 0, 0} : tics_vec3_negate(tics_vec3_normalize(q_vec));
	...
	tics_vec3 local = tics_vec3_sub(point, rotation_center);
	// "move" the point according to the angular velocity
	tics_vec3 rotated =
		tics_quat_rotate_vec3(local, tics_quat_from_axis_angle(axis, half_angle * 2.0f));
	tics_vec3 rotated_world_space = tics_vec3_add(rotated, rotation_center);

	// ws_premoved_point = rotated_world_space + lin_vel * 0.1f
	tics_vec3 ws_premoved_point =
		tics_vec3_add(rotated_world_space, tics_vec3_mul_f(lin_vel, 0.1f));

	// total_v = (ws_premoved_point - point) * 10.0f
	tics_vec3 total_v = tics_vec3_mul_f(tics_vec3_sub(ws_premoved_point, point), 10.0f);
	return total_v;
}
...
static void solve_impulses(tics_world* world, collision* collisions, float delta) {
	...
	for (size_t i = 0; i < count; ++i) {
		...
		tics_vec3 r_a = tics_vec3_sub(col->result.point_a, pos_a);
		tics_vec3 r_b = tics_vec3_sub(col->result.point_b, pos_b);

		float r_a_dist_squared = tics_vec3_length_sq(r_a);
		float r_b_dist_squared = tics_vec3_length_sq(r_b);

		tics_vec3 n = col->result.normal;

		tics_vec3 v_r = tics_vec3_sub(velocity_a, velocity_b);
		// relative velocity in the collision normal direction
		float n_dot_vr = tics_vec3_dot(v_r, n);
		...
		float denom = inv_mass_a + inv_mass_b + tics_vec3_dot(n, tics_vec3_add(term1, term2));
		...
		tics_vec3 normal_comp = tics_vec3_mul_f(n, tics_vec3_dot(v_r, n));
		tics_vec3 collision_tangent = tics_vec3_normalize(tics_vec3_sub(v_r, normal_comp));

		tics_vec3 friction_impulse =
			tics_vec3_mul_f(collision_tangent, impulse_magnitude * dynamic_friction_coefficient);

		// impulse = (magnitude * n) - friction
		tics_vec3 impulse = tics_vec3_sub(tics_vec3_mul_f(n, impulse_magnitude), friction_impulse);

		// apply impulses only to rigid bodies
		if (rb_a) {
			rb_a->impulse = tics_vec3_add(rb_a->impulse, impulse);

			tics_vec3 angular_impulse = tics_vec3_cross(r_a, impulse);
			if (angular_impulse.x != 0 || angular_impulse.y != 0 || angular_impulse.z != 0) {
				float str = tics_vec3_length(angular_impulse) * 0.1f / r_a_dist_squared;
				tics_vec3 axis = tics_vec3_normalize(angular_impulse);
				rb_a->an_imp_div_sq_dst = tics_quat_from_axis_angle(tics_vec3_negate(axis), str);
			}
		}
		if (rb_b) {
			impulse = tics_vec3_negate(impulse); // apply impulse in opposite direction
			rb_b->impulse = tics_vec3_add(rb_b->impulse, impulse);

			tics_vec3 angular_impulse = tics_vec3_cross(r_b, impulse);
			if (angular_impulse.x != 0 || angular_impulse.y != 0 || angular_impulse.z != 0) {
				float str = tics_vec3_length(angular_impulse) * 0.1f / r_b_dist_squared;
				tics_vec3 axis = tics_vec3_normalize(angular_impulse);
...
static void solve_positions(tics_world* world, collision* collisions, float delta) {
	...
	for (size_t i = 0; i < count; ++i) {
		...
		tics_vec3 correction = tics_vec3_mul_f(col->result.normal, percent * depth_with_tolerance);
		...
		if (rb_a && rb_b) {
			...
			tics_vec3 offset_a = tics_vec3_mul_f(correction, b_share);
			tics_vec3 offset_b = tics_vec3_mul_f(correction, -(1.0f - b_share));

			rb_a->transform.position = tics_vec3_add(rb_a->transform.position, offset_a);
			rb_b->transform.position = tics_vec3_add(rb_b->transform.position, offset_b);
		}
		else if (rb_a && sb_b) {
			// rigid body vs static body (Only move rigid body A)
			rb_a->transform.position = tics_vec3_add(rb_a->transform.position, correction);
		}
		else if (sb_a && rb_b) {
			// static body vs RigidBody (Only move rigid body B)
			tics_vec3 neg_correction = tics_vec3_negate(correction);
			rb_b->transform.position = tics_vec3_add(rb_b->transform.position, neg_correction);
		}
	}
}
